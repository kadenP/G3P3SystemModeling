...
model G3P3_System 
  ...
  input Real hotBin_Tout;
  input Real hotBin_m_dot_out;
  input Real hotBin_m_s;
  ...
  FallingParticleReceiverSystem.SourceSink.ParticleSink HotBinSink annotation(
        Placement(visible = true, transformation(origin = {10, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  FallingParticleReceiverSystem.SourceSink.ParticleSource HotBinSource annotation(
...
equation
  ...
  connect(HotBinInlet.ParticleOutlet, HotBinSink.ParticleInlet) annotation(
    Line(points = {{10, 100}, {10, 80}}));
  connect(HotBinSource.ParticleOutlet, HotBinDischarge.ParticleInlet) annotation(
...
// Heat Exchanger Flow Rate
  ...
  CO2Inlet.m_dot = 5;
// Hot Bin Source
  HotBinSource.T = hotBin_Tout;
  HotBinSource.m_dot = hotBin_m_dot_out;
// Receiver Flow Rate
...
// Heat Exchanger Operation
      when initial() then
        HX_m_dot := 1E-10;
        Heater_m_dot := 0.1;
      elsewhen hotBin_m_s > 10000 then
        HX_m_dot := 5.0;
      elsewhen hotBin_m_s < 1000 then
        HX_m_dot := 1E-10;
      end when;
// Receiver Operation
      when initial() then
